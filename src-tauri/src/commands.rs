...
pub fn get_db_schemas(db: State<Database>) -> Result<Vec<DbSchema>, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, name, description, data, created_at, updated_at FROM db_schemas ORDER BY created_at DESC")
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn create_db_schema(db: State<Database>, input: CreateDbSchemaInput) -> Result<DbSchema, String> {
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
    let id = generate_id();
    let timestamp = now();
...
#[tauri::command]
pub fn update_db_schema(db: State<Database>, input: UpdateDbSchemaInput) -> Result<DbSchema, String> {
    let conn = db.conn.lock().map_err(|e| e.to_string())?;
    let timestamp = now();
